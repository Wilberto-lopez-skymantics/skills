...
description: "Build iterative WYSIWYG HTML wireframes from DESIGN.md design tokens and SPEC.md component hierarchy without external API dependencies."
---
...
Bridge between requirements gathering (Brainstorming) & production implementation (Dev Swarm). Relies purely on Agent code-generation → HTML. Zero external API dependencies.
...
6. **Transition** — Return control to `spec-driven-development` orchestrator.
...
- Files missing | incomplete → user needs `brainstorming` skill first
...
*See [visual-companion.md](file://{{SKILLS_DIR}}/shared/visual-companion.md) for detailed server instructions.*
...
3. User requests change → **directly edit HTML using `replace_file_content`**
...
Inform user wireframing complete, WYSIWYG specs locked in. Return control to `spec-driven-development` orchestrator. ⊥ invoke downstream skills directly. SDD determines next step (typically `writing-implementation-phases`).
